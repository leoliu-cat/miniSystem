...
import dns from "dns";
import { setupWebsiteTables, mountWebsiteApi } from "./src/api-website";
...
async function startServer() {
  ...
  app.post("/api/auth/send-verification-code", async (req, res) => {
    try {
      ...
      await db.prepare("INSERT INTO email_verification_codes (email, code, expires_at) VALUES (?, ?, ?)").run(email, code, expires_at);

      const subject = "您的網站驗證碼";
      const body = `您的驗證碼是：${code}\n驗證碼有效期限為 10 分鐘，請盡快輸入。`;

      if (process.env.RESEND_API_KEY) {
        console.log("[EMAIL] Sending verification code via Resend API to", email);
        ...
        console.log(`[EMAIL] Simulated sending verification code to ${email}: ${code}`);
      }
  ...
  app.post("/api/auth/verify-code", async (req, res) => {
    try {
      ...
      const record = await db.prepare("SELECT * FROM email_verification_codes WHERE email = ? ORDER BY id DESC LIMIT 1").get(email) as any;
      if (!record) {
        return res.status(400).json({ error: "未找到驗證碼記錄，請重新發送" });
      }
      ...
      // Once verified, we can generate a temporary frontend token if needed, 
      // but for simple verification, just return success
  ...
  app.post("/api/send-email", authenticateToken, async (req, res) => {
    try {
      ...
      if (resendKey) {
        ...
        if (!response.ok) {
          const errData = await response.json().catch(() => ({}));
          throw new Error(`SendGrid API Error: ${response.status} ${JSON.stringify(errData)}`);
        }
        return res.json({ success: true, method: 'sendgrid' });

      } else if (!smtpHost) {
        console.warn("[EMAIL] No SMTP_HOST or API Keys configured. Simulation only.");
        return res.json({ success: true, method: 'simulation' });
      }

      console.log("[EMAIL] Sending via general SMTP...");
      const transporter = nodemailer.createTransport({
      ...
      await transporter.sendMail({
        from: process.env.SMTP_FROM || '"系統通知" <test@example.com>',
  ...
  // ==========================================
  app.post("/api/admin/upload", authenticateToken, (req, res, next) => {
    upload.single("file")(req, res, (err) => {
      if (err) {
        console.error("Upload error:", err.message);
        return res.status(400).json({ error: "檔案上傳失敗: " + err.message });
      }
      next();
    });
  }, async (req, res) => {
  ...
  app.get("/uploads/:filename", async (req, res) => {
    try {
      ...
      if (isS3) {
        ...
        const uploadPath = path.join(DATA_DIR, "uploads", filename);
