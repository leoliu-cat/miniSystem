...
async function startServer() {
  ...
  const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
      ...
      cb(null, "template-" + uniqueSuffix + (ext === '.pdf' ? '.pdf' : '.pdf'));
    },
  });
  const upload = multer({ 
    storage: storage,
  ...
  const db = new Database(path.join(DATA_DIR, "weddings.db"));
  db.exec(`
  ...
  const subjectSetting = db.prepare("SELECT value FROM settings WHERE key = 'email_subject'").get();
  if (!subjectSetting) {
    db.prepare("INSERT INTO settings (key, value) VALUES ('email_subject', ?)").run(defaultSubjectTemplate);
  }
  const bodySetting = db.prepare("SELECT value FROM settings WHERE key = 'email_body'").get();
  if (!bodySetting) {
    db.prepare("INSERT INTO settings (key, value) VALUES ('email_body', ?)").run(defaultBodyTemplate);
  }
  const marketingSubjectSetting = db.prepare("SELECT value FROM settings WHERE key = 'marketing_email_subject'").get();
  if (!marketingSubjectSetting) {
    db.prepare("INSERT INTO settings (key, value) VALUES ('marketing_email_subject', ?)").run(defaultMarketingSubject);
  }
  const marketingBodySetting = db.prepare("SELECT value FROM settings WHERE key = 'marketing_email_body'").get();
  if (!marketingBodySetting) {
    db.prepare("INSERT INTO settings (key, value) VALUES ('marketing_email_body', ?)").run(defaultMarketingBody);
  }
  const tagsSetting = db.prepare("SELECT value FROM settings WHERE key = 'recommended_tags'").get();
  if (!tagsSetting) {
    db.prepare("INSERT INTO settings (key, value) VALUES ('recommended_tags', ?)").run("已買書約,潛在彌月客,VIP,急件,海外客戶,需特別注意");
  }

  // Migration: Add new columns if they don't exist
  const columns = db.prepare("PRAGMA table_info(weddings)").all() as any[];
  ...
  for (const col of newColumns) {
    if (!columnNames.includes(col.name)) {
      db.exec(`ALTER TABLE weddings ADD COLUMN ${col.name} ${col.type}`);
    }
  }
  ...
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_order_code ON weddings(order_code) WHERE order_code IS NOT NULL;");

  // Migration: delete default templates
  db.prepare("DELETE FROM templates WHERE id IN ('template1', 'template2')").run();
  ...
  const incomeColumns = db.prepare("PRAGMA table_info(incomes)").all() as any[];
  if (!incomeColumns.some(c => c.name === 'invoice_number')) {
    db.exec("ALTER TABLE incomes ADD COLUMN invoice_number TEXT");
  }
  ...
  const designerCount = db.prepare("SELECT COUNT(*) as count FROM designers").get() as any;
  ...
  const designerColumns = db.prepare("PRAGMA table_info(designers)").all() as any[];
  const designerColumnNames = designerColumns.map(c => c.name);
  if (!designerColumnNames.includes('role')) {
    db.prepare("ALTER TABLE designers ADD COLUMN role TEXT DEFAULT 'designer'").run();
  }

  // Ensure admin account exists
  const adminExists = db.prepare("SELECT id FROM designers WHERE username = 'admin'").get();
  if (!adminExists) {
    const adminPassword = bcrypt.hashSync("admin123", 10);
    db.prepare("INSERT INTO designers (name, username, password, role) VALUES (?, ?, ?, ?)").run("管理員", "admin", adminPassword, "admin");
  }
  ...
  app.post("/api/login", (req, res) => {
    try {
      ...
      const designer = db.prepare("SELECT * FROM designers WHERE username = ?").get(username) as any;
  ...
  app.get("/api/designers", authenticateToken, (req, res) => {
    try {
      const designers = db.prepare("SELECT id, name, username FROM designers").all();
      res.json(designers);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/designers", authenticateToken, (req, res) => {
    try {
      const { name, username, password } = req.body;
      if (!name || !username || !password) {
        return res.status(400).json({ error: "請填寫所有欄位" });
      }
      const existing = db.prepare("SELECT id FROM designers WHERE username = ?").get(username);
      if (existing) {
        return res.status(400).json({ error: "此帳號已存在" });
      }
      const hashedPassword = bcrypt.hashSync(password, 10);
      const info = db.prepare("INSERT INTO designers (name, username, password) VALUES (?, ?, ?)").run(name, username, hashedPassword);
  ...
  app.put("/api/designers/:id", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      ...
      if (password) {
        const hashedPassword = bcrypt.hashSync(password, 10);
        db.prepare("UPDATE designers SET name = ?, password = ? WHERE id = ?").run(name, hashedPassword, id);
      } else {
        db.prepare("UPDATE designers SET name = ? WHERE id = ?").run(name, id);
      }
  ...
  app.delete("/api/designers/:id", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      db.prepare("DELETE FROM designers WHERE id = ?").run(id);
      // Also unassign this designer from any weddings
      db.prepare("UPDATE weddings SET designer_id = NULL WHERE designer_id = ?").run(id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/settings", authenticateToken, (req, res) => {
    try {
      ...
      db.transaction(() => {
        if (email_subject !== undefined) stmt.run('email_subject', email_subject);
        if (email_body !== undefined) stmt.run('email_body', email_body);
        if (recommended_tags !== undefined) stmt.run('recommended_tags', recommended_tags);
        if (quotation_packages !== undefined) stmt.run('quotation_packages', quotation_packages);
        if (quotation_illustrators !== undefined) stmt.run('quotation_illustrators', quotation_illustrators);
        if (quotation_addons !== undefined) stmt.run('quotation_addons', quotation_addons);
  ...
  app.post("/api/templates", authenticateToken, upload.single("file"), (req, res) => {
    try {
      const file = req.file;
      ...
      stmt.run(id, name, file.filename, null);

      res.json({ success: true, id, name, filename: file.filename });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });

  app.put("/api/templates/:id", authenticateToken, upload.single("file"), (req, res) => {
    try {
      const { id } = req.params;
      const { name } = req.body;
      ...
      const currentTemplate = db.prepare("SELECT * FROM templates WHERE id = ?").get(id) as any;
      ...
      stmt.run(name, newFilename, null, id);
  ...
  app.delete("/api/weddings/:id", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/orders/:id/duplicate", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      const order = db.prepare("SELECT * FROM weddings WHERE id = ?").get(id) as any;
  ...
  app.get("/api/orders/code/:orderCode", (req, res) => {
    try {
      const { orderCode } = req.params;
      const stmt = db.prepare("SELECT * FROM weddings WHERE order_code = ?");
      const order = stmt.get(orderCode);
  ...
  app.post("/api/orders/code/:orderCode/details", (req, res) => {
    try {
      const { orderCode } = req.params;
      const {
  ...
  app.put("/api/orders/:id/status", authenticateToken, (req, res) => {
    try {
      ...
      db.prepare("UPDATE weddings SET status = ? WHERE id = ?").run(status, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/orders/:id/tracking", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      const { tracking_number } = req.body;
      db.prepare("UPDATE weddings SET tracking_number = ? WHERE id = ?").run(tracking_number, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/orders/:id/payment", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      const { payment_date, amount } = req.body;
      const tax = Math.round((amount || 0) * 0.05);
      db.prepare("UPDATE weddings SET payment_date = ?, amount = ?, tax = ? WHERE id = ?").run(payment_date, amount, tax, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/orders/:id/dates", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      const { design_deadline, delivery_date } = req.body;
      db.prepare("UPDATE weddings SET design_deadline = ?, delivery_date = ? WHERE id = ?").run(design_deadline, delivery_date, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/expenses", authenticateToken, (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `);
      const info = stmt.run(expense_date, item_name, amount, category, notes);
      res.json({ success: true, id: info.lastInsertRowid });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/expenses/:id", authenticateToken, (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        WHERE id = ?
      `);
      stmt.run(expense_date, item_name, amount, category, notes, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/incomes", authenticateToken, (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);
      const result = stmt.run(income_date, item_name, amount, tax || 0, bank_last_5 || '', notes || '', invoice_number || '');
  ...
  app.put("/api/incomes/:id", authenticateToken, (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        WHERE id = ?
      `);
      stmt.run(income_date, item_name, amount, tax || 0, bank_last_5 || '', notes || '', invoice_number || '', id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.get("/api/quotations/:id", (req, res) => {
    try {
      ...
      const quotation = stmt.get(req.params.id);
      if (quotation) {
        res.json(quotation);
      } else {
  ...
  app.post("/api/quotations", authenticateToken, (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      const result = stmt.run(customer_name, ig_handle, email, phone, wedding_date, delivery_date, quotation_data, total_amount, notes);
  ...
  app.put("/api/quotations/:id/status", authenticateToken, (req, res) => {
    try {
      const { status } = req.body;
  ...
  app.post("/api/weddings/:id/marketing-email", authenticateToken, async (req, res) => {
    try {
      ...
      const wedding = db.prepare("SELECT * FROM weddings WHERE id = ?").get(id) as any;
      if (!wedding) {
      ...
      db.prepare("INSERT INTO marketing_email_logs (wedding_id) VALUES (?)").run(id);
  ...
  app.get("/api/unsubscribe/:id", async (req, res) => {
    try {
      const { id } = req.params;
      db.prepare("UPDATE weddings SET unsubscribed = 1 WHERE id = ?").run(id);
      res.send(`
        <html>
          <head>
            <meta charset="utf-8">
  ...
  app.put("/api/weddings/:id/unsubscribe", authenticateToken, async (req, res) => {
    try {
      ...
      db.prepare("UPDATE weddings SET unsubscribed = ? WHERE id = ?").run(unsubscribed ? 1 : 0, id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/weddings/:id/marketing-logs", authenticateToken, async (req, res) => {
    try {
      const { id } = req.params;
      const logs = db.prepare("SELECT * FROM marketing_email_logs WHERE wedding_id = ? ORDER BY sent_at DESC").all(id);
      res.json(logs);
    } catch (error) {
  ...
  app.get("/api/weddings", authenticateToken, (req, res) => {
    try {
      const stmt = db.prepare(`
        SELECT w.*, 
  ...
  app.put("/api/weddings/:id/assign", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      const { designer_id } = req.body;
      db.prepare("UPDATE weddings SET designer_id = ? WHERE id = ?").run(designer_id, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/weddings/:id/status", authenticateToken, (req, res) => {
    try {
      const { id } = req.params;
      const { status } = req.body;
      db.prepare("UPDATE weddings SET status = ? WHERE id = ?").run(status, id);
  ...
  app.get("/api/weddings/:id/download-txt", authenticateToken, async (req, res) => {
    try {
      ...
      const wedding = stmt.get(id) as any;
  ...
  // Serve templates statically for thumbnails
  app.use("/templates", (req, res, next) => {
    if (req.path.endsWith('.svgz')) {
      res.setHeader('Content-Encoding', 'gzip');
      res.setHeader('Content-Type', 'image/svg+xml');
    }
    next();
  }, express.static(path.join(DATA_DIR, "templates")));
